...
class MNISTNet:
	...
	def __init__(self, NN, batch_size=50, step_size=1e-5, MNIST_directory = './MNIST_data/', use_gpu = False, iter_per_print=None):
		self.NN = NN
	...
	def train(self, iterations, time_cutoff = None):
		...
		for i in range(iterations):
			...
			if time_cutoff != None and time.time() - current_time > time_cutoff:
				break
	...
	def test(self):
		...
		for datum in self.test_data:
			...
			_, pred = torch.max(self.NN.forward(image),1)
